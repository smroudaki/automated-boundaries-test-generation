...
class CFG(object):
    ...
    def _visit_blocks(self, graph, block, visited=[], calls=True):
        ...
        if 'return' in nodelabel:  # Not efficient for processing
            end_nodes.append(block.id)
        nodes.append(block.id)
        ...
        for exit in block.exits:
            ...
            edges.append([block.id, exit.target.id])
