...
def reachHead(path, graph):
    ...
    a simple path.
    """
    former_nodes = [n for n in graph['nodes'] if path[0] in graph[
        'edges'][n]]
...
def findSimplePath(graph, exPaths, paths=[]):
    ...
    paths.extend([p for p in exPaths if isPrimePath(p, graph)])
    exPaths = [p for p in exPaths if extendable(p, graph)]
    newExPaths = []
    for p in exPaths:
